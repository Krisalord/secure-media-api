...
import io.github.krisalord.integration.core.getRefreshTokenCookie
...
class AuthApiTest : BaseIntegrationTest() {
    ...
    @Test
    fun `login - should fail with 401 Unauthorized when email does not exist`() = runSecureTestApplication { client ->
        val loginResponse = client.post("/api/v1/auth/login") {
            contentType(ContentType.Application.Json)
            setBody("""{"email": "ghost_user@example.com", "password": "Password123!"}""")
        }
    ...
    fun `refresh - should issue new tokens when a valid refresh cookie is provided`() = runSecureTestApplication { client ->
        ...
        assertNotNull(refreshResponse.body<AuthTokenResponse>().accessToken)
